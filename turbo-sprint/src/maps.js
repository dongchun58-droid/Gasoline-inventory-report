...
export const MAPS = {
  ...
  ice: {
    ...
    // 하단 해안: 바깥쪽(+)만 바다(side=1). 안쪽(-)은 육지(눈밭·나무). 이탈 방향이 바다일 때만 추락.
    seaEdges: [[0.66, 0.985, 1]],
    obstacle: 'snowball',
    // 정상 점프대: 진행방향이 착지점(t≈0.63)을 정확히 향하는 지점(t≈0.48)에서 도약 → 옆/뒤로 안 밀리고
    // '앞으로' 큰 포물선을 그리며 자연스럽게 날아감(정상 직전, 높이 게이트로 조기발동 방지).
    pad: { boost: 0x8fe0ff, chevron: '#eaffff', jump: '#4ad6ff', jumpHex: 0x4ad6ff, jumpEdge: 0xffffff,
      jumps: [0.48], boosts: [0.05, 0.22, 0.30, 0.38, 0.44, 0.90],
