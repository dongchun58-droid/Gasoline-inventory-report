...
def fetch_eia_gasoline_stocks(api_key):
...
def year_to_array(week_dict, n=52):
...
def build_chart(by_year, cur_year):
    ...
    for yr in past_years:
        ...
        ax.plot(xs, ys, color=PAST_COLORS.get(yr, "#aaaaaa"), linewidth=1.3, zorder=2)
...
def build_pdf(chart_png, by_year, cur_year):
    ...
    story.append(Paragraph(
        ...
        "Weekly Petroleum Status Report (WGTSTUS1). "
        "Data through latest available week.", s_src))
    ...
    obs = [
        ...
        "visible across all vintage years. The current year line is plotted against the "
        f"2022-{cur_year-1} historical band for seasonal comparison.",
    ...
    story.append(Paragraph(
        ...
        "All figures in million barrels. "
        "Five-year average excludes 2020. For informational purposes only.",
        s_src))
...
<div style="max-width:680px;margin:0 auto;background:#ffffff;">
  ...
  <div style="background:#f7f9fc;padding:16px 32px;border-top:1px solid #e0e0e0;">
    <p style="color:#999;font-size:11px;margin:0;line-height:1.5;">
      Source: U.S. EIA Weekly Petroleum Status Report (WGTSTUS1).
      Figures in million barrels. For informational purposes only.</p>
  </div>
</div>
</body>
</html>"""
...
if __name__ == "__main__":
    ...
    if cur_year in by_year:
        ...
        print(f"      {cur_year} latest: W{latest_wk} = {latest_mb} mb")
