...
export class GameAudio {
  ...
  // ---- ZOMBIE SURGE 효과음 ----
  sfxShot() { if (!this.ctx) return; const t = this.ctx.currentTime; this._burst(t, 0.045, 0.18); this._tone('square', 180, 90, t, 0.05, 0.12); }
  sfxCard(type) { if (!this.ctx) return; const t = this.ctx.currentTime; if (type === 'minus') { this._tone('sawtooth', 300, 120, t, 0.25, 0.4); } else { [0, 5, 9].forEach((n, i) => this._tone('square', 660 * Math.pow(2, n / 12), null, t + i * 0.06, 0.12, 0.35)); } }
  sfxZombieDie() { if (!this.ctx) return; const t = this.ctx.currentTime; if (this._zdCd && t < this._zdCd) return; this._zdCd = t + 0.08; this._tone('sawtooth', 140, 60, t, 0.14, 0.18); }
  sfxHurt() { if (!this.ctx) return; const t = this.ctx.currentTime; if (this._huCd && t < this._huCd) return; this._huCd = t + 0.12; this._tone('square', 220, 110, t, 0.12, 0.25); }
  sfxRoar() { if (!this.ctx) return; const t = this.ctx.currentTime; this._tone('sawtooth', 90, 45, t, 1.1, 0.7); this._burst(t, 0.6, 0.5); }
  sfxBossDie() { if (!this.ctx) return; const t = this.ctx.currentTime; this._tone('sawtooth', 120, 30, t, 1.4, 0.7); this._burst(t, 0.9, 0.7); }
  sfxFail() { if (!this.ctx) return; const t = this.ctx.currentTime; [0, -3, -7, -12].forEach((n, i) => this._tone('square', 330 * Math.pow(2, n / 12), null, t + i * 0.18, 0.22, 0.4)); }
}
